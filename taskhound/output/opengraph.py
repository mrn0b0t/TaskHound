...
def _get_bloodhound_token(api_url: str, username: str, password: str) -> Optional[str]:
    ...
    try:
        token = get_bloodhound_token(api_url, username, password)
        debug(f"Successfully authenticated to BloodHound at {api_url}")
        return token

    except requests.Timeout:
        warn("Timeout authenticating to BloodHound (>30s)")
        return None
...
def resolve_object_ids_chunked(
    ...
    1. Check Cache for existing mappings
    2. If computer_sids provided: Query by objectId (SID) - most reliable!
    3. Otherwise: Query by name using WHERE IN clause
    4. Build name→(node_id, objectId) mappings from results
    5. If API fails, fallback to LDAP for missing entries (provides objectId only)
    ...
    # 1. Check Cache for Computers
    cached_computers = set()
    for name in computer_names:
        cached_val = cache.get("computer_node", name)
        if cached_val:
            try:
                node_data = json.loads(cached_val)
                # node_data is [node_id, object_id]
                computer_sid_map[name] = (node_data[0], node_data[1])
                cached_computers.add(name)
                debug(f"Cache hit for computer {name} -> {node_data[1]}")
    ...
    # 2. Check Cache for Users
    cached_users = set()
    for name in user_names:
        cached_val = cache.get("user_node", name)
        if cached_val:
            try:
                node_data = json.loads(cached_val)
                user_sid_map[name] = (node_data[0], node_data[1])
                cached_users.add(name)
                debug(f"Cache hit for user {name} -> {node_data[1]}")
            except json.JSONDecodeError:
                pass
    ...
    def _query_bloodhound_with_sid_validation(
        names_with_sids: Dict[str, str], node_type: str
        ...
        try:
            ...
            if bh_connector.api_key and bh_connector.api_key_id:
                ...
                response = requests.post(
                    f"{base_url}/api/v2/graphs/cypher", headers=headers, json=query_data, timeout=bh_connector.timeout
                )
            else:
    ...
    if remaining_computers:
        info(f"Resolving {len(remaining_computers)} computer names to objectIds...")
        ...
        if computer_sids:
            ...
            computers_with_sids = {
                name: sid for name, sid in computer_sids.items() if name in remaining_computers and sid
            }
            ...
            if computers_with_sids:
                ...
                for i, chunk in enumerate(name_chunks, 1):
                    ...
                    for name, val in chunk_mapping.items():
                        computer_sid_map[name] = val
                        cache.set("computer_node", name, json.dumps(val))
        ...
        for i, chunk in enumerate(computer_chunks, 1):
            ...
            if missing_names:
                info(
                    f"Chunk {i}/{len(computer_chunks)}: {len(chunk_mapping)}/{len(chunk)} resolved via API, {len(missing_names)} missing"
                )
    ...
    if remaining_users:
        info(f"Resolving {len(remaining_users)} user names to objectIds...")
        user_chunks = _chunk_list(remaining_users, chunk_size)
        ...
        for i, chunk in enumerate(user_chunks, 1):
            ...
            # Update map and cache
            for name, val in chunk_mapping.items():
                user_sid_map[name] = val
                cache.set("user_node", name, json.dumps(val))
            ...
            if missing_names:
                info(
                    f"Chunk {i}/{len(user_chunks)}: {len(chunk_mapping)}/{len(chunk)} resolved via API, {len(missing_names)} missing"
                )
...
def generate_opengraph_files(
    ...
    if total_skipped["computers"] > 0 or total_skipped["users"] > 0:
        warn("Skipped edges due to missing nodes:")
        ...
        info("Tip: Use --allow-orphans to create edges to missing nodes (may create orphaned edges)")
...
def _create_task_node(task: Dict) -> Node:
    ...
    node = Node(id=object_id, kinds=["scheduledtask", "Base", "TaskHound"], properties=Properties(**properties_dict))
...
def _create_relationship_edges(
    ...
    if computer_map and hostname in computer_map:
        ...
        if not allow_orphans:
            ...
            has_task_edge = Edge(
                start_node=hostname, end_node=task_object_id, kind=edge_kind, start_match_by="name", end_match_by="id"
            )
            edges.append(has_task_edge)
