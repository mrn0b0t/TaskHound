...
class TestCrawlFailure:
    ...
    def test_crawl_failure_silent_disappearance(self, mock_smb):
        ...
        # Call process_target
        auth = AuthContext(username="user", password="password", domain="DOMAIN")
        process_target(
            target=target,
            all_rows=all_rows,
            auth=auth,
        ...
        # Check that we show the failure reason - could be truncated by Rich table
        # The new table format shows the reason directly
        assert "Access Denied" in output or "Admin check" in output or "FAILED HOSTS" in output
